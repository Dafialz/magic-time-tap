...
export default function App() {
  ...
  // ===== Offline popup (заміняє window.alert)
  ...
  const toggleEquip = (id: string) => {
    setEquippedIds(prev => {
      ...
      if (prev.length >= 3) { setOfflineModalText("Макс. 3 артефакти одночасно"); setOfflineModalOpen(true); return prev; }
      return [...prev, id];
    });
  };
  ...
  return (
    <div className="app" style={{ minHeight: "100vh", background: "transparent" }}>
        ...
        {activeTab === "skins" && (
          <SkinsShop
            ...
            buySkin={(id: string, price: number) => {
              ...
              if (mm < price) { setOfflineModalText("Не вистачає MM"); setOfflineModalOpen(true); return; }
              setMm(v => v - price);
      ...
      {/* Глобальна модалка для офлайн/системних повідомлень */}
      <AppModal
